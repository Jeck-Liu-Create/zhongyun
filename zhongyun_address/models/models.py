...
class ZyAddress(models.Model):
    ...
    # 起运地
    port_state = fields.Many2one('res.country.state', string='省', required=True, domain=[('country_id', '=', 48)],
                                 copy=True)
    ...
    # 止运地
    stop_state = fields.Many2one('res.country.state', string='省', required=True, domain=[('country_id', '=', 48)],
                                 copy=True)
